...
```{r}
rank3pm <- select(nba06To11, c("rank", "lost"))
pairs(rank3pm)
```
...
sample.split(df$AnyColumn, SplitRatio = decimal)
```{r}
# Set the random seed for repeatability
set.seed(123) 

# Split the data into 3:1 ratio
sample = sample.split(nba06To11$rank, SplitRatio = .75)
train = subset(nba06To11, sample == TRUE)
test  = subset(nba06To11, sample == FALSE)
```

### Build Model on Training Set
```{r}
rankLostModel <- lm(rank ~ lost, data = train)
rankLostModel
```
...
predictedRank <- predict(rankLostModel, test)
predictedRank

# Create data frame with actual and predicted values
rankPrediction <- data.frame(Lost = test$lost,
                             ActualRank = test$rank,
...
testSet <- data.frame(lost = c(28))
# Predict the position with the model and test data
predictedRank <- predict(rankLostModel, testSet)
predictedRank
```
...
```{r}
ggplot(data = rankPrediction, mapping = aes(x = Lost)) +
  geom_point(data = rankPrediction, 
             mapping = aes(y = ActualRank),
             color = "blue") +
  geom_point(data = rankPrediction, 
             mapping = aes(y = PredictedRank), 
             color = "red") +
  labs(title = "Games Lost vs Rank" ,
