...
```{r}
ggplot() + 
  geom_point(data = avg3pa, aes(x = Year, y = Average2PA), color = "blue") +
  geom_point(data = avg3pa, aes(x = Year, y = Average3PA), color = "red") +
  theme(axis.text.x = element_text(angle =- 90, vjust = 0.5)) +
...
teamSeries = ts(avg3pa[, 2], frequency = 1)
...
ggplot(data = wssResults, mapping = aes(x= k, y= wss)) +
  geom_point() +
  geom_line() +
  labs(title = "K-means: Subject Grades", x = "Number of Clusters k", y = "Within Sum of Squares")
```
